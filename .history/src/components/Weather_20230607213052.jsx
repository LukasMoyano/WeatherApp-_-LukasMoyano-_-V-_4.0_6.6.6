...
const Weather = ({ weatherInfo }) => {
  ...
  return (
    <section className="bg-white bg-opacity-70 p-4 rounded-lg text-center">
        ...
        <article className="mb-4 md:mb-0">
          ...
          <div className="relative">
            {weatherInfo?.weather[0].icon ? (
              <img
                src={`https://openweathermap.org/img/wn/${weatherInfo?.weather[0].icon}@4x.png`}alt="Weather Icon"className="mx-auto h-24"/>
            ) : null}
            {/* Superposición para efecto de sombreado */}
            <div className="absolute inset-0 bg-black opacity-50 rounded-lg"></div>
          </div>
          <h3 className="text-base mt-4 mb-2">
            {weatherInfo?.weather[0].description} {/* Mostrar la descripción del clima */}
          </h3>
          <span className="text-4xl font-bold">
            {convertTemperature(weatherInfo?.main.temp)} {/* Mostrar la temperatura convertida */}
          </span>
        </article>
          ...
          >
            C°/F°
          </button> {/* Botón para cambiar la unidad de temperatura */}
          ...
          <section className="grid grid-cols-3 gap-4 md:flex md:flex-row md:items-center">
            <article className="flex items-center justify-center flex-col">
              <div className="flex items-center">
                ...
                <span className="text-lg">{weatherInfo?.wind.speed}m/s</span> {/* Mostrar la velocidad del viento */}
              </div>
            </article>
            ...
            <article className="flex items-center justify-center flex-col">
              <div className="flex items-center">
                ...
                <span className="text-lg">{weatherInfo?.main.humidity}%</span> {/* Mostrar la humedad */}
              </div>
            </article>
            ...
            <article className="flex items-center justify-center flex-col">
              <div className="flex items-center">
                ...
                <span className="text-lg">{weatherInfo?.main.pressure}hPa</span> {/* Mostrar la presión */}
              </div>
            </article>
          </section>
        </div>
